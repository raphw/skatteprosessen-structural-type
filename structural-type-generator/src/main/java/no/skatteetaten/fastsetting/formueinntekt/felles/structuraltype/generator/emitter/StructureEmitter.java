...
import no.skatteetaten.fastsetting.formueinntekt.felles.structuraltype.api.StructureReference;
...
public class StructureEmitter implements BiConsumer<CompoundDescription, Map<String, CompoundDescription.Property>> {
    ...
    public void accept(CompoundDescription compound, Map<String, CompoundDescription.Property> properties) {
        ...
        TypeSpec.Builder builder = TypeSpec.interfaceBuilder(structure).addModifiers(Modifier.PUBLIC).addAnnotation(AnnotationSpec.builder(CompoundOf.class).addMember(
            "value",
        ...
        if (implementationGenerations.contains(ImplementationGeneration.PROJECTION)) {
            ...
            if (featureGenerations.contains(FeatureGeneration.FACTORY_ON_STRUCTURE)) {
                CodeBlock.Builder ofAny = CodeBlock.builder()
                    .beginControlFlow("if (value == null)")
                    .addStatement("throw new $T()", NullPointerException.class);
                CodeBlock.Builder ofType = CodeBlock.builder()
                ...
                compound.getSingulars().forEach(singular -> {
                    builder.addMethod(MethodSpec.methodBuilder("of")
                        ...
                        .addCode(CodeBlock.builder()
                            .addStatement("return $T.wrap(value)", nameResolver.projection(compound, singular))
                            .build())
                        .build());
                    ofAny.nextControlFlow("else if (value instanceof $T)", singular.getType()).addStatement(
                        "return $T.of(of(($T) value))", Optional.class, singular.getType()
                    );
                    if (!singular.isLeaf() && !java.lang.reflect.Modifier.isAbstract(singular.getType().getModifiers())) {
                        ofType.nextControlFlow("else if (type == $T.class)", singular.getType()).addStatement(
                ...
                builder.addMethod(MethodSpec.methodBuilder("ofAny")
                    ...
                    .addCode(ofAny.nextControlFlow("else")
                ...
                builder.addMethod(MethodSpec.methodBuilder("ofType")
                    ...
                    .addCode(ofType.nextControlFlow("else")
                        .addStatement("return $T.empty()", Optional.class)
                        .endControlFlow()
                        .build())
                    .build());
        ...
        if (featureGenerations.contains(FeatureGeneration.COPY)) {
            ...
            if (implementationGenerations.contains(ImplementationGeneration.PROJECTION)) {
                ...
                CodeBlock.Builder code = CodeBlock.builder()
                    ...
                    .addStatement("throw new $T()", NullPointerException.class);
                compound.getSingulars().stream().filter(singular -> !singular.isLeaf()).forEach(singular -> code
                    .nextControlFlow("else if (target == $T.class)", singular.getType())
                    .addStatement("return $T.copyOf(this, copies)", nameResolver.projection(compound, singular)));
                builder.addMethod(MethodSpec.methodBuilder("copyTo")
                    .addModifiers(Modifier.PUBLIC, Modifier.DEFAULT)
                    .addParameter(ParameterizedTypeName.get(
                        ClassName.get(Class.class), WildcardTypeName.subtypeOf(TypeName.OBJECT)
                    ...
                    .returns(structure)
                    .addCode(code.nextControlFlow("else").addStatement(
                        "throw new $T($S + $T.class.getTypeName() + $S + target.getTypeName())",
        ...
        properties.forEach((name, property) -> property.accept((cardinality, type) -> addProperty(
            builder, structure, false, name, cardinality, TypeName.get(type), null
    ...
    private void addProperty(
        ...
        if (!name.isEmpty()) {
            ...
            if (propertyGenerations.contains(PropertyGeneration.FLUENT)) {
                ...
                if (propertyGenerations.contains(PropertyGeneration.TRIAL)) {
                    builder.addMethod(MethodSpec.methodBuilder(propertyResolver.accessor(structure, name, PropertyGeneration.FLUENT))
                        .returns(structure)
                        .addModifiers(Modifier.PUBLIC, Modifier.DEFAULT)
                        ...
                        .addCode(CodeBlock.builder().addStatement(
                            "this.$N($N).ifPresent($N -> $N.accept(this, $N))",
                            propertyResolver.accessor(structure, name, PropertyGeneration.TRIAL),
                            name,
                        ...
                        .build());
                    if (expansion != null) {
                        expansion.getDescription().accept(expansionType -> builder.addMethod(MethodSpec
                            .methodBuilder(propertyResolver.accessor(structure, name, PropertyGeneration.FLUENT))
                            .returns(structure)
                            .addModifiers(Modifier.PUBLIC, Modifier.DEFAULT)
                            .addParameter(expansionType, name)
                            .addParameter(ParameterizedTypeName.get(
                                ClassName.get(BiConsumer.class), WildcardTypeName.supertypeOf(structure), WildcardTypeName.supertypeOf(RuntimeException.class)),
                                name.equals("fallback") ? "fallback0" : "fallback"
                            )
                            ...
                            .build()
                        ), ignored -> builder.addMethod(MethodSpec.methodBuilder(propertyResolver.accessor(structure, name, PropertyGeneration.FLUENT))
                            .returns(structure)
                            .addModifiers(Modifier.PUBLIC, Modifier.DEFAULT)
                            .addParameter(nameResolver.structure(expansion.getDescription()), name)
                            .addParameter(ParameterizedTypeName.get(
                                ClassName.get(BiConsumer.class), WildcardTypeName.supertypeOf(structure), WildcardTypeName.supertypeOf(RuntimeException.class)),
                                name.equals("fallback") ? "fallback0" : "fallback"
                            )
            ...
            if (propertyGenerations.contains(PropertyGeneration.CLEAR)) {
                builder.addMethod(propertyResolver.clear(structure, name, type));
            }
