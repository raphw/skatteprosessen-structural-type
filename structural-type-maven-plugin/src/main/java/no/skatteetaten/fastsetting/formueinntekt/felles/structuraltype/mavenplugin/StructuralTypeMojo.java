    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try (URLClassLoader classLoader = new URLClassLoader(
            ...
            if (discover) {
                ...
                try (ScanResult result = new ClassGraph().addClassLoader(classLoader).filterClasspathElementsByURL(
                    element -> !url.equals(element)
            ...
            List<BiPredicate<Class<?>, String>> exclusions = filters.stream()
                ...
                .map(entry -> (BiPredicate<Class<?>, String>) ((type, property) -> entry.getKey().equals(type.getTypeName()) && entry.getValue().stream().anyMatch(
            ...
            StructuralType structuralType = new StructuralType()
                ...
                .withNamingStrategy(DecoratingNamingStrategy.withDuplicationResolution(DecoratingNamingStrategy.withReplacements(
                    new FilteringNamingStrategy(
                        new CommonPrefixNamingStrategy(1, subpackage),
                        true,
                        type -> subordinations == null || !subordinations.contains(type.getTypeName())
                    ),
                ...
                try {
                    structuralType = structuralType.withStructuralResolver(
                        jaxb == JaxbHandler.JAVAX
                            ...
                            : JaxbStructuralResolver.ofJakarta(classLoader)
                    ).withAccessResolver(
                        jaxb == JaxbHandler.JAVAX
            ...
            structuralType = structuralType.withNodeResolver((type, property) -> Optional.ofNullable(translationByClass
